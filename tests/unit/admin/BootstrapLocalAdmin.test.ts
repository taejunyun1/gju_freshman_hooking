...
describe('local administrator bootstrap guard', () => {
  ...
  it('creates an active password administrator without TOTP enrollment guidance', async () => {
    const { bootstrapLocalAdmin } = await import('../../../scripts/bootstrap-local-admin')
    const write = vi.fn()
    const adapter = {
    ...
    expect(adapter.createUser).toHaveBeenCalledWith('local-admin@example.test', 'environment-only-password')
    ...
    expect(adapter.upsertAdmin).toHaveBeenCalledWith('admin-1')
    expect(write).toHaveBeenCalledWith('Local administrator ready. Sign in at /admin/login.')
    expect(JSON.stringify(write.mock.calls)).not.toMatch(/TOTP|enroll|secret=|environment-only-password/iu)
  })
})
