...
import { adminCounselingAssignSchema, type AdminCounselingCurrent } from '../../../../../shared/schemas/counseling'
import {
...
type AssignHandlerDependencies = {
  counseling: Pick<ReturnType<typeof getServerAdminCounselingService>, 'assign'>
...
export const createAssignCounselingHandler = (dependencies: AssignHandlerDependencies) => async (
  event: unknown,
  ...
  try {
    ...
    const input = parseAdminCounselingJsonBody({
      ...
      schema: adminCounselingAssignSchema,
    })
    return {
      data: await dependencies.counseling.assign(publicId, input, {
...
export default defineEventHandler(event => createAssignCounselingHandler({
