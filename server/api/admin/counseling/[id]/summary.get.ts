...
type SummaryHandlerDependencies = {
  counseling: Pick<ReturnType<typeof getServerAdminCounselingService>, 'summary'>
...
export const createCounselingSummaryHandler = (dependencies: SummaryHandlerDependencies) => async (
  event: unknown,
): Promise<ApiSuccess<{ summary: string }> | ApiFailure> => {
  ...
  try {
    ...
    return {
      data: {
        summary: await dependencies.counseling.summary(publicId, {
...
export default defineEventHandler(event => createCounselingSummaryHandler({
