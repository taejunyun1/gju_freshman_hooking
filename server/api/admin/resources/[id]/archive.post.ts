...
import {
  ...
  parseAdminResourceTransition,
} from '../../../../modules/admin/resources'
import { AppError, toApiFailure } from '../../../../utils/app-error'
import {
  readBoundedRequestBody,
  RequestBodyLimitError,
  RESOURCE_TRANSITION_MAX_REQUEST_BODY_BYTES,
} from '../../../../utils/bounded-request-body'

type ResourcesService = ReturnType<typeof getServerAdminResourcesService>
type Response = Awaited<ReturnType<ResourcesService['archive']>>
type Dependencies = {
  resources: Pick<ResourcesService, 'archive'>
...
export const createArchiveAdminResourceHandler = (dependencies: Dependencies) => async (
  event: unknown,
  ...
  try {
    ...
    const input = parseAdminResourceTransition(body)
    return {
      data: await dependencies.resources.archive(
        id,
...
export default defineEventHandler(event => createArchiveAdminResourceHandler({
  ...
  readRawBody: requestEvent => readBoundedRequestBody(requestEvent, RESOURCE_TRANSITION_MAX_REQUEST_BODY_BYTES),
