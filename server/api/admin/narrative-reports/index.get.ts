...
import type { AdminNarrativeReportList } from '../../../../shared/schemas/admin-narrative-reports'
import {
...
type ListHandlerDependencies = {
  ...
  reports: Pick<ReturnType<typeof getServerAdminNarrativeReportsService>, 'list'>
...
export const createAdminNarrativeReportsListHandler = (
  dependencies: ListHandlerDependencies,
) => async (
  event: unknown,
): Promise<ApiSuccess<AdminNarrativeReportList> | ApiFailure> => {
  ...
  try {
    ...
    const query = parseAdminNarrativeReportListQuery(dependencies.getQuery(event))
    return { data: await dependencies.reports.list(query), requestId }
  }
  catch (error) {
...
export default defineEventHandler(event => createAdminNarrativeReportsListHandler({
