...
type Dependencies = {
  commands: Pick<Commands, 'add'>
  getBody: (event: unknown) => Promise<unknown>
  getRequestId: (event: unknown) => string
  requireAdmin: (event: unknown) => { userId: string } | Promise<{ userId: string }>
  setHeader: (event: unknown, name: string, value: string) => void
  setStatus: (event: unknown, status: number) => void
}

export const createAdminRosterStudentAddHandler = (dependencies: Dependencies) => async (
  event: unknown,
): Promise<ApiSuccess<Awaited<ReturnType<Commands['add']>>> | ApiFailure> => {
  ...
  try {
    ...
    return { data: await dependencies.commands.add(await dependencies.getBody(event), { adminUserId: admin.userId }), requestId }
  }
  catch (error) {
...
export default defineEventHandler(event => createAdminRosterStudentAddHandler({
