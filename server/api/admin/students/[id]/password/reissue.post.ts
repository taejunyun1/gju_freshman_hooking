...
type Dependencies = {
  commands: Pick<Commands, 'reissuePassword'>
  ...
  requireRecentAdmin: (event: unknown, options: { recentAuthMinutes: number }) => unknown
  setHeader: (event: unknown, name: string, value: string) => void
  setStatus: (event: unknown, status: number) => void
}

export const createAdminRosterStudentPasswordReissueHandler = (dependencies: Dependencies) => async (
  event: unknown,
): Promise<ApiSuccess<Awaited<ReturnType<Commands['reissuePassword']>>> | ApiFailure> => {
  ...
  try {
    dependencies.requireRecentAdmin(event, { recentAuthMinutes: 15 })
    return {
      data: await dependencies.commands.reissuePassword({
...
export default defineEventHandler(event => createAdminRosterStudentPasswordReissueHandler({
  ...
  getRequestId: requestEvent => typeof (requestEvent as { context?: { requestId?: unknown } }).context?.requestId === 'string'
    ...
    : crypto.randomUUID(),
  requireRecentAdmin: (requestEvent, options) => requireRecentAdminContext(requestEvent, options.recentAuthMinutes),
