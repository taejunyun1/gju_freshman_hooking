...
type Dependencies = {
  commands: Pick<Commands, 'changePhone'>
...
export const createAdminRosterStudentPhoneHandler = (dependencies: Dependencies) => async (
  event: unknown,
): Promise<ApiSuccess<Awaited<ReturnType<Commands['changePhone']>>> | ApiFailure> => {
  ...
  try {
    ...
    return {
      data: await dependencies.commands.changePhone({
...
export default defineEventHandler(event => createAdminRosterStudentPhoneHandler({
