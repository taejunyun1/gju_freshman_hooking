...
import {
  getServerAdminExportService,
  parseExportJobId,
  parseExportPageQuery,
...
type Response = Awaited<ReturnType<Service['students']>>
type Dependencies = {
  exports: Pick<Service, 'students'>
...
export const createAdminExportStudentsHandler = (dependencies: Dependencies) => async (
  event: unknown,
  ...
  try {
    ...
    return { data: await dependencies.exports.students(jobId, cursor, { adminUserId: admin.userId }), requestId }
  }
  catch (error) {
...
export default defineEventHandler(event => createAdminExportStudentsHandler({
