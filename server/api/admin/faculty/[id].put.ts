...
import { getServerAdminFacultyService, parseAdminFacultyId, parseAdminFacultyJsonBody, parseAdminFacultyUpdate } from '../../../modules/admin/faculty'
...
type Response = Awaited<ReturnType<Service['update']>>
type Dependencies = {
  faculty: Pick<Service, 'update'>
...
export const createUpdateAdminFacultyHandler = (dependencies: Dependencies) => async (
  event: unknown,
  ...
  try {
    ...
    return { data: await dependencies.faculty.update(id, parseAdminFacultyUpdate(raw), { adminUserId: admin.userId, requestId }), requestId }
  }
  catch (error) {
...
export default defineEventHandler(event => createUpdateAdminFacultyHandler({
