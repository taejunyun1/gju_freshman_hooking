...
<style scoped>
.roster-page { display: grid; gap: 1.2rem; }
.roster-page__header { display: flex; flex-wrap: wrap; justify-content: space-between; gap: 1rem; align-items: end; }
.roster-page__header p { margin: 0 0 0.6rem; color: var(--color-primary); font-family: var(--font-mono); font-size: 0.67rem; font-weight: 700; letter-spacing: 0.09em; }
.roster-page h1 { margin: 0; font-family: var(--font-display); font-size: clamp(1.75rem, 4vw, 2rem); letter-spacing: -0.05em; }
.roster-page__header span { display: block; margin-top: 0.7rem; color: var(--color-muted); }
button { min-height: var(--touch-target); border: 1px solid var(--color-primary); border-radius: var(--radius-control); background: var(--color-surface); color: var(--color-primary-strong); padding: 0.55rem 0.75rem; font-family: var(--font-display); font-weight: 700; cursor: pointer; }
.roster-page__start { display: flex; flex-wrap: wrap; gap: 0.6rem; align-items: end; border: 1px solid color-mix(in srgb, var(--color-primary) 20%, transparent); border-radius: var(--radius-panel); background: var(--color-surface); padding: 1rem; }
.roster-page__start label { display: grid; gap: 0.3rem; font-family: var(--font-mono); font-size: 0.7rem; }
.roster-page__start input { min-height: var(--touch-target); border: 1px solid var(--color-primary); border-radius: var(--radius-control); padding: 0.35rem; background: var(--color-surface); }
.roster-page__start button { background: var(--color-primary); color: var(--color-surface); }
.roster-page__counts { display: flex; flex-wrap: wrap; align-items: center; gap: 0.5rem; border: 1px solid color-mix(in srgb, var(--color-primary) 18%, transparent); border-radius: var(--radius-panel); background: var(--color-surface); padding: 0.75rem 1rem; font-family: var(--font-mono); font-size: 0.72rem; }
.roster-page__counts strong { padding-right: 0.5rem; border-right: 1px solid color-mix(in srgb, var(--color-primary) 24%, transparent); }
.roster-page__counts small { color: var(--color-muted); }
.roster-page__credential { display: flex; flex-wrap: wrap; gap: 0.6rem; align-items: center; }
.roster-page__credential span { font-size: 0.78rem; color: var(--color-muted); }
button:focus-visible { outline: 3px solid var(--color-primary); outline-offset: 3px; }
@media (max-width: 34rem) {
  .roster-page__header { align-items: start; }
  .roster-page__header > button { width: 100%; }
  .roster-page__counts { display: grid; grid-template-columns: 1fr 1fr; }
  .roster-page__counts strong { border: 0; }
}
</style>
