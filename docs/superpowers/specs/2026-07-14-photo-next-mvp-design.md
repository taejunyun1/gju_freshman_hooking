...
핵심 시그니처는 영상 편집 타임라인을 닮은 **연결 근거 타임라인**이다. 학생이 선택한 관심사는 클립으로 놓이고, 각 클립이 다음 레인으로 이어진다.

```text
[선택 관심사 클립]
          |
          v
V1 교과목 ──> V2 장비·시설 ──> V3 비교과·프로젝트 ──> OUT 작품·진로
  왜 연결?       어디서 실습?         어떻게 확장?            무엇이 되는가?
```

타임라인은 장식이 아니라 실제 추천 데이터의 순서와 연결 이유를 표현한다. V1 교과 레인 안에서는 추천 교과를 1학년 기초 → 2학년 제작·후반 → 3학년 트랙 워크숍 → 4학년 포트폴리오·현장으로 다시 묶어, 관심사가 4년 동안 어떻게 심화되는지도 보여준다. 일반적인 보라색 SaaS 대시보드처럼 보이지 않도록 그라디언트 KPI 히어로, 의미 없는 유리 카드, 모든 요소의 과도한 둥근 모서리는 사용하지 않는다. 학생 화면에는 차가운 캔버스와 편집 레인 구조를, 관리자 화면에는 절제된 표와 상태 표시를 사용한다. 한 화면의 강한 시각 요소는 타임라인 하나로 제한한다.
...
V1 교과목      V2 장비·시설      V3 비교과       OUT
스튜디오조명 → 대형 스튜디오 → 브랜드 프로젝트 → 광고 포트폴리오
선택한 '패션 화보'와 '인물 연출'을 실제 조명 실습으로 연결합니다.
...
각 범주 점수는 표시 대상으로 선정된 자원 적합도의 가중 평균이다. 해당 범주의 승인 데이터가 전혀 없으면 전체 점수를 과장하지 않도록 그 범주를 0으로 계산하고 관리자 데이터 품질 경고를 남긴다. 장학금과 일반 지원 제도는 결과에 표시하지만 점수에는 넣지 않는다.
