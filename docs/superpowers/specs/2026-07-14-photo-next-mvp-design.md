...
- 관리자 승인은 `(복구 요청 ID, 관리자 UUID, 요청 trace UUID)` RPC 한 트랜잭션에서 상태 전이와 `{}` metadata 감사 기록을 완료한 뒤에만 원문 코드를 한 번 반환한다.
- 성공 시 사용한 요청은 `consumed`, 같은 학생의 다른 `requested|verified` 요청은 `expired`로 만들고 모든 활성 코드 해시와 학생 세션을 같은 트랜잭션에서 폐기한다.

### 9.3 로그인 제한

- 계정 기준 실패 5회 시 15분 잠금
- IP 기준 로그인 10회/5분, 번호 HMAC 기준 5회/5분
- 가입 5회/IP/1시간, 복구 요청 3회/번호 HMAC/1시간과 10회/IP/1시간
- 복구 완료 시도 10회/IP/5분과 5회/코드 해시/15분을 PBKDF2 파생 전에 적용
