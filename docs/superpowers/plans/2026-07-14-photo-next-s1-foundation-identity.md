...
Add the forward-only `202607140007_login_hardening.sql` migration. Login completion locks the credential row first and compares the exact password hash and salt verified by the application before resetting failures or inserting a session. Only the new proof-bound signature is executable by `service_role`; the legacy signature is revoked.

Every non-safe `/api/student/*` browser request requires an exact same-origin `Origin`. Canonicalize repeated and trailing slashes before selecting CSRF-protected routes and reject encoded student mutation paths. Anonymous registration, login, recovery request, and recovery completion require no session CSRF. Cookie-authenticated logout and password change additionally send `X-Photo-Next-CSRF`, a fixed-format HMAC-SHA-256 token derived from the opaque session and exposed only by `/api/student/session` for page-memory use. Rate-limit IP keys use a single validated `CF-Connecting-IP` only in Cloudflare runtime, otherwise a direct unforwarded client address or `unknown`; never trust X-Forwarded-For. Student and administrator API responses send `Cache-Control: private, no-store`.
...
`bootstrap-local-admin.ts` creates only the documented local test email from environment input, validates password sign-in, and then upserts its Auth UUID into `admin_users`. It does not enroll an MFA factor or print an enrollment URI, QR payload, secret, or password; it prints one non-secret instruction to enroll TOTP on the first browser login. The browser uses the supported Supabase Auth MFA enrollment and challenge/verify flow, and Supabase Auth generates the TOTP secret; do not patch Auth internals or attempt unsupported caller-selected secrets. Before starting a replacement enrollment, the browser removes every stale unverified TOTP factor returned in `factors.all` and aborts without enrolling if any removal fails. The explicit “다시 로그인” action likewise removes the current unverified factor before local sign-out, and stays in the retryable enrollment state with a generic error if removal fails. The script aborts unless the Supabase URL is localhost and never persists authentication input or output. The runbook directs both local and production operators to enroll TOTP on first browser login; production operators create the real Auth user in Supabase Dashboard, copy only the Auth UUID into `admin_users`, and disable public admin signup. No password or TOTP secret is written to the repository.
